...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, user_id: str, token: str) -> bool:
        ...
        logger.debug(f"Connected (auth skipped) with {websocket.client}")
        return True
...
async def websocket_endpoint(
    ...
    try:
        ...
        # Try to fetch user data from the DB (skip auth fallback for local)
        user_data = await agent_db.get(["mint_user_id"]) or {"mint_user_id": "test_user"}

        agent = AgentMint(
            user_id=user_id,
            mint_user_id=user_data.get("mint_user_id", "test_user"),
            chat_id=chat_id,
            ip_addr=websocket.client.host,
            is_advanced=advanced,
        )
