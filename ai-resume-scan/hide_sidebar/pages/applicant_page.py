...
def save_prediction_to_supabase(resume_filename_str, job_title_str, analysis_result_dict, model_used_str):
    """Stores resume analysis output in the Supabase prediction_history table."""
    if not supabase_client:
        st.error("Supabase client is not initialized — unable to save record.")
        print("Supabase client missing in save_prediction_to_supabase.")
        return False
    ...
    new_entry = {
        ...
        "missing_skills_list": ", ".join(analysis_result_dict.get('missing_skills', [])),
    ...
    try:
        ...
        if response.data:
            print(f"Successfully added prediction history entry. ID: {response.data[0].get('id')}")
            return True
        else:
            error_message = "Unknown error during insertion."
            if hasattr(response, 'error') and response.error:
                error_message = response.error.message
            st.error(f"Failed to store prediction history in Supabase: {error_message}")
            print(f"Supabase save failed. Error: {error_message}, Full response: {response}")
            return False
    except Exception as e:
        st.error(f"Unexpected error while saving record: {e}")
...
def run():
    ...
    st.markdown("<div class='big-font'>Upload Resume & Get Instant Job Match</div>", unsafe_allow_html=True)
    st.markdown("<div class='small-muted'>Pick an AI model to evaluate your resume with the chosen job post.</div>", unsafe_allow_html=True)
    st.markdown("---")

    if "job_data" not in st.session_state or not st.session_state.job_data:
        st.warning("No job selected! Please pick one from the Home section.")
    ...
    with st.expander("Job Description Overview", expanded=True):
    ...
    st.markdown("---")
    st.subheader("Upload Resume & Select Analysis Model")

    model_options_list = [MODEL_GEMINI_PRO, MODEL_LSTM_CUSTOM, MODEL_TRANSFORMER_CUSTOM, MODEL_RULE_BASED]
    model_captions_list = [
        "In-depth AI review powered by Gemini Pro.",
        "AI-based LSTM model offering an overall compatibility score.",
        "Fine-tuned Transformer model generating AI-backed scores.",
        "Rule-based method for quick and simple analysis."
    ]
    ...
    if selected_model_name == MODEL_GEMINI_PRO:
        st.info("Gemini Pro will be used for advanced AI-driven evaluation.")
    elif selected_model_name == MODEL_LSTM_CUSTOM:
        st.info("The LSTM model predicts a total score; details come from rule-based logic.")
    elif selected_model_name == MODEL_TRANSFORMER_CUSTOM:
        st.info("Transformer model predicts AI-based overall compatibility; rule-based details included.")
    else:
        st.info("Running quick rule-based comparison on skills and experience.")

    with st.form("resume_upload_form_applicant"):
        resume_file_uploaded = st.file_uploader(
            "Select Resume (PDF, DOCX, DOC, or TXT)",
            type=["pdf", "docx", "doc", "txt"],
            key="resume_uploader_applicant_v2"
        )
        analyze_button = st.form_submit_button("Start Analysis")
    ...
    if analyze_button:
        if not resume_file_uploaded:
            ...
            progress_bar_analysis = st.progress(0, text="Initializing analysis...")

            with st.spinner("Extracting resume content..."):
                progress_bar_analysis.progress(25, text="Extracting resume data...")
                parsed_resume_data = parse_resume(resume_file_uploaded)
                time.sleep(0.2)

            if not parsed_resume_data or not parsed_resume_data.get("raw_text"):
                st.error("Failed to extract text content from the uploaded resume.")
                progress_bar_analysis.empty()
            else:
                with st.spinner(f"Matching resume using {selected_model_name}..."):
                    progress_bar_analysis.progress(65, text=f"Running model: {selected_model_name}...")
                ...
                progress_bar_analysis.progress(100, text="Completed successfully!")
                st.success("Resume analysis finished!")
                st.toast("Results are ready — scroll down to view insights!")
    ...
    if st.session_state.get("show_applicant_results_v2", False) and "analysis_output_applicant" in st.session_state:
        ...
        st.markdown("---")
        st.subheader(f"Analysis Results (Model: {model_used_for_display})")
        ...
        with res_col2:
            st.metric("Skill Alignment", f"{skill_score_val}%", delta_color="off")
            st.progress(int(skill_score_val) / 100)

        st.metric("Experience Relevance", f"{exp_score_val}%", delta_color="off")
        ...
        if missing_skills_items:
            ...
            st.success("All key skills appear to be aligned with job requirements!")
        ...
        if model_used_for_display == MODEL_GEMINI_PRO and gemini_summary_val:
            with st.expander("Gemini Pro Deep Analysis", expanded=True):
                st.markdown("**Suitability Overview:**")
            ...
            st.info("No additional suggestions generated for this resume.")
        ...
        try:
            ...
            fig_breakdown = px.pie(
                ...
                values=pie_values,
                title="Breakdown of Score Components"
            )
            st.plotly_chart(fig_breakdown, use_container_width=True)
        except ValueError:
            st.caption("Could not plot breakdown chart — score values not numeric.")
                    ...
                    try:
                        ...
                        if st.session_state.resume_ref.type == "application/pdf":
                            if resume_bytes:
                                ...
                                st.warning("The PDF file appears to be empty.")
                            ...
                            st.warning("File format not supported for preview.")
                        ...
                        st.error(f"Unexpected error while previewing resume: {e}")
                else:
                    st.info("Please upload your resume to view it here.")
