...
def load_prediction_history_from_supabase() -> pd.DataFrame:
    """Fetch prediction history from the Supabase table."""
    if not supabase_client:
        st.error("Supabase client is not initialized. Unable to fetch prediction history.")
    ...
    try:
        ...
        st.error(f"Error fetching prediction history from Supabase: {e}")
        print(f"Error fetching prediction history from Supabase: {e}")
        return pd.DataFrame()

def run():
    st.title("Dashboard: Prediction Insights and Model Evaluation")
    st.markdown("Review resume-job match results, assess model performance, and track trends over time.")
    st.markdown("---")

    df = load_prediction_history_from_supabase()

    if df.empty:
        st.info("No prediction history available. Please analyze resumes in the Applicant Portal to view data.")
        return
    ...
    if 'model_used' in filtered_df.columns and 'match_score' in filtered_df.columns:
        ...
        st.info("Not enough data to display model comparison table.")
    ...
    if not filtered_df.empty and 'model_used' in filtered_df.columns and 'match_score' in filtered_df.columns:
        ...
        if not model_avg_scores.empty:
            ...
            st.info("Not enough data to display average match scores.")
    ...
    if not filtered_df.empty and 'model_used' in filtered_df.columns and 'match_score' in filtered_df.columns:
        if len(filtered_df["model_used"].unique()) > 0:
            ...
            st.info("Not enough distinct models to display score distribution.")
    ...
    if not filtered_df.empty and 'timestamp' in filtered_df.columns and 'match_score' in filtered_df.columns and 'model_used' in filtered_df.columns:
        if len(filtered_df) > 1:
            ...
            st.info("Only one record available; trend analysis requires multiple data points.")
        else:
    ...
    if 'missing_skills_list' in filtered_df.columns:
        ...
        st.info("'missing_skills_list' column not available in the dataset.")

    st.markdown("---")
    if not df.empty:
        csv_export = df.to_csv(index=False).encode('utf-8')
        st.download_button(
            label="Download Full Prediction History (CSV)",
            data=csv_export,
