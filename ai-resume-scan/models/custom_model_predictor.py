...
def load_lstm_model_and_tokenizer():
    """Load the LSTM model and its tokenizer if not already loaded."""
    ...
    if _loaded_lstm_model is None:
        if os.path.exists(LSTM_MODEL_PATH):
            ...
            print(f"LSTM model file does not exist at: {LSTM_MODEL_PATH}")
            return False
    ...
    if _loaded_lstm_tokenizer is None:
        if os.path.exists(LSTM_TOKENIZER_PATH):
            try:
                ...
                print("LSTM tokenizer successfully loaded.")
            except Exception as e:
...
def predict_with_lstm(resume_text, job_text):
    """Generate a match score prediction using the LSTM model."""
    if not load_lstm_model_and_tokenizer():
        print("LSTM model/tokenizer unavailable.")
        return None
...
def load_transformer_model_and_tokenizer():
    """Load the Transformer model and tokenizer if not already loaded."""
    global _loaded_transformer_model, _loaded_transformer_tokenizer

    if not os.path.exists(TRANSFORMER_MODEL_PATH):
        print(f"Transformer model directory does not exist at {TRANSFORMER_MODEL_PATH}")
        return False
    ...
    if _loaded_transformer_model is None:
        try:
            ...
            print(f"Transformer model loaded on {_transformer_device}")
        except Exception as e:
    ...
    if _loaded_transformer_tokenizer is None:
        try:
            ...
            print("Transformer tokenizer successfully loaded.")
        except Exception as e:
            print(f"Error loading Transformer tokenizer: {e}")
            return False

    return True

def predict_with_transformer(resume_text, job_text):
    """Generate a match score prediction using the Transformer model."""
    if not load_transformer_model_and_tokenizer():
        print("Transformer model/tokenizer unavailable.")
        return None

    try:
        combined_text = resume_text + " [SEP] " + job_text
        inputs = _loaded_transformer_tokenizer(combined_text, return_tensors="pt", padding=True, truncation=True, max_length=512)
