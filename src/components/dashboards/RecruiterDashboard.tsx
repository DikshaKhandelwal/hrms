...
export const RecruiterDashboard = ({ activeView = 'dashboard' }: RecruiterDashboardProps) => {
  ...
  useEffect(() => {
    ...
    // Subscribe to realtime changes on attendance and leave_requests so
    // updates from other browser sessions are reflected here.
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          console.debug('realtime attendance event', payload);
          loadAttendanceStats();
          // notify in-page listeners as well
      ...
      .on(
        ...
        (payload) => {
          console.debug('realtime leave_requests event', payload);
          // notify components that listen for leave changes
          window.dispatchEvent(new Event('leave-request-updated'));
          // attendance stats may be impacted by approved leaves
          loadAttendanceStats();
        }
      )
      .subscribe();
              ...
              <div className="flex gap-2">
                {['Today', 'Weekly', 'Monthly', 'Custom'].map((filter) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded text-sm font-medium ${timeFilter === filter
