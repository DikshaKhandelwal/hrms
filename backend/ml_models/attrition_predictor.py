...
class AttritionPerformancePredictor:
    ...
    def _engineer_features(self, df):
        """Engineer same features as training (EnhancedPreprocessor)"""
        # TenureRatio
        ...
        # WorkLifeScore
        cols = ['JobSatisfaction', 'WorkLifeBalance', 'EnvironmentSatisfaction']
        if all(c in df.columns for c in cols):
            df['WorkLifeScore'] = df[cols].sum(axis=1)
        ...
        if all(c in df.columns for c in sat_cols):
            df['OverallSatisfaction'] = df[sat_cols].mean(axis=1)
        ...
        if 'MonthlyIncome' in df.columns:
            df['BelowMedianIncome'] = (df['MonthlyIncome'] < 6500).astype(int)  # Use training median
        ...
        if 'YearsSinceLastPromotion' in df.columns and 'YearsAtCompany' in df.columns:
            df['StagnantCareer'] = ((df['YearsSinceLastPromotion'] > 5) & (df['YearsAtCompany'] > 5)).astype(int)
        ...
        if 'Age' in df.columns:
            df['AgeGroup'] = pd.cut(df['Age'], bins=[0, 30, 40, 50, 100], labels=[0, 1, 2, 3])
            df['AgeGroup'] = df['AgeGroup'].fillna(1).astype(int)
        
        # Experience levels
        if 'TotalWorkingYears' in df.columns:
            df['ExperienceLevel'] = pd.cut(df['TotalWorkingYears'], bins=[0, 5, 10, 20, 100], labels=[0, 1, 2, 3])
    ...
    def predict_attrition(self, employee_data: Dict) -> Dict:
        ...
        try:
            ...
            # Use saved preprocessor components to transform data
            ...
            for col, encoder in self.preprocessor['label_encoders'].items():
                if col in ml_df.columns:
                    try:
                        ...
                        ml_df[col] = encoder.transform([encoder.classes_[0]])[0]
            
            # Engineer additional features (same as training)
            ml_df = self._engineer_features(ml_df)
            
            # Select features in correct order
            feature_cols = [f for f in self.preprocessor['feature_names'] if f in ml_df.columns]
            missing_cols = [f for f in self.preprocessor['feature_names'] if f not in ml_df.columns]
            ...
            ml_df = ml_df[self.preprocessor['feature_names']]
            
            # Scale features
            X = self.preprocessor['scaler'].transform(ml_df)
...
# Create global predictor instance for API
